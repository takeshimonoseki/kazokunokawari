...
6.  **ログシートへ処理ログ保存**: 処理の各段階（受信、保存、通知結果）でログシートに情報を記録
...
-   **管理者メール送信**: 処理結果（「未送信」「送信済み」「送信しない」「エラー」）を記録
-   **自動返信メール送信**: 処理結果（「未送信」「送信済み」「送信しない」「エラー」）を記録
-   **LINE通知送信**: 処理結果（「未送信」「送信済み」「送信しない」「エラー」）を記録
...
-   送信元情報（「家族の代わり（軽貨物便利屋TAKE）」運営名、電話、メール）を記載
...
-   **Sheets保存エラー**: Google Sheetsへのデータ保存でエラーが発生した場合。`500 Internal Server Error`を返し、ログシートに`ERROR`レベルで記録。依頼・相談シートの`エラー内容`列に詳細を記録。
-   **管理者メール送信エラー**: 管理者通知メールの送信でエラーが発生した場合。`INFO`レベルでログを記録し、依頼・相談シートの`管理者メール送信`列に「エラー」を記録するが、フォーム受付自体は正常扱いとする。
-   **自動返信エラー**: 自動返信メールの送信でエラーが発生した場合。`INFO`レベルでログを記録し、依頼・相談シートの`自動返信メール送信`列に「エラー」を記録するが、フォーム受付自体は正常扱いとする。
-   **LINE通知エラー**: LINE通知の送信でエラーが発生した場合。`INFO`レベルでログを記録し、依頼・相談シートの`LINE通知送信`列に「エラー」を記録するが、フォーム受付自体は正常扱いとする。

### 方針
-   Google Sheetsへのデータ保存を最優先する。Sheetsへの保存が完了した場合は、その他の通知処理（メール、LINE）でエラーが出ても、原則としてフォーム受付自体は正常扱いとし、ユーザーには正常レスポンスを返す。
-   通知処理のエラーはログシートに記録し、依頼・相談シートの該当列にも「エラー」と記録することで、後から管理者が手動でフォローアップできるようにする。
-   `GAS_ENV` Script Propertyが`development`の場合、エラーレスポンスに`details`フィールドを含めることでデバッグを容易にする。
